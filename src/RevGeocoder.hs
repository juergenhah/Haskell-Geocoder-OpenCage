...
reverseGeocode latt long developerKey = reverseGeocodeWith params
  where
    params =
      defaultParams
        {para_latitude = latt, para_longitude = long, para_key = developerKey}
...
getReverseAPIResponseWith parameters = do
  ...
  case reqSuccesfull of
    Left err -> do
      putStrLn "exception at geoCage request, issue:"
      print err
      return Nothing
    Right response ->
...
convertQParamstoOptionsList (QueryParameters para_lat para_long key lang pre jsonp) =
  ...
  toParameter "pretty" pre ++ toParameter "jsonp" jsonp
  where
    toParameter :: String -> Maybe T.Text -> [(String, String)]
    toParameter param mValue =
      case mValue of
        Nothing -> []
...
instance FromJSON Result where
  parseJSON (Object v) =
    Result <$> v .: "annotations" <*> v .: "components" <*> v .: "confidence" <*>
    v .:? "formatted" .!= "" <*>
    v .: "geometry"
...
instance FromJSON Components where
  parseJSON (Object v) =
    Components <$> (v .:? "country" .!= "") <*> (v .:? "country_code" .!= "") <*>
    (v .:? "county" .!= "") <*>
    (v .:? "state" .!= "") <*>
    (v .:? "city" .!= "") <*>
    (v .:? "road" .!= "") <*>
